...
def setup_logging():
    ...
    # Define the path for the mu_editor folder inside Documents
    MU_EDITOR_DIR = os.path.join(DOCUMENTS_DIR, "mu_editor")

    # Create the mu_editor directory if it doesn't exist
    if not os.path.exists(MU_EDITOR_DIR):
        os.makedirs(MU_EDITOR_DIR)

    # Define subdirectories for storing files and libraries within the mu_editor folder
    LIBRARIES_DIR = os.path.join(MU_EDITOR_DIR, "libraries")
    FILES_DIR = os.path.join(MU_EDITOR_DIR, "files")
