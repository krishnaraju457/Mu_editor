...
class AdafruitMode(MicroPythonMode):
    ...
    if not comports:
        logger.error("No CircuitPython boards found")
        board_info = "No CircuitPython boards found"
        
    for comport in comports:
        print(f"Found CircuitPython Board:")
        print(f"Device Path: {comport.device}")
        print(f"VID: {hex(comport.vid)}")
        print(f"PID: {hex(comport.pid)}")
        print(f"Serial Number: {comport.serial_number if comport.serial_number else 'N/A'}")
        valid_boards.append((comport.vid,comport.pid))
        show_board_info()
